...
import com.real.umssimulator.UserController;
...
privileged aspect UserController_Roo_Controller_Json {
    ...
    @RequestMapping(params = "find=ByGuidEquals", headers = "Accept=application/json")
    @ResponseBody
    public ResponseEntity<String> UserController.jsonFindUsersByGuidEquals(@RequestParam("guid") String guid) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(User.toJsonArray(User.findUsersByGuidEquals(guid).getResultList()), headers, HttpStatus.OK);
    }
    
    @RequestMapping(params = "find=ByTransaction_idEquals", headers = "Accept=application/json")
    @ResponseBody
    public ResponseEntity<String> UserController.jsonFindUsersByTransaction_idEquals(@RequestParam("transaction_id") String transaction_id) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(User.toJsonArray(User.findUsersByTransaction_idEquals(transaction_id).getResultList()), headers, HttpStatus.OK);
    }
